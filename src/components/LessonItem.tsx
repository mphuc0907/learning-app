    ...
    >
      <div className="flex items-center justify-between gap-3 max-sm:flex-col max-sm:items-start">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-medium max-sm:text-sm">{lesson.title}</div>
            <div className="text-xs text-gray-500">{lesson.duration} phút</div>
          </div>
        </div>
        <div className="flex items-end justify-end float-end w-full">
        <span className={`text-xs px-2 py-1 text-right block rounded-full border ${completed ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'text-gray-600 border-gray-200'}`}>
